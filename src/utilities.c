...
/**
 * Returns matrix raised to the power of n.
 *
 * @param size The size of the matrix. Matrix is assumed to be square so only one size is needed.
 * @param matrix The matrix to be raised to the power.
 * @param n A power greater or equal to 1.
 * @param res The matrix that the result is stored in.
 */
void matrixPower(const int size, const int* matrix, int n, int* res)  {
    ...
    if(n > 1) {
        ...
        /** TODO: need another array that holds intermediate results because res gets changed and then changed values are
         *  used again in the future which results in wrong answers
         */
        int total = 0;
            ...
            for(int row = 0; row < size; row++) {
                for(int col = 0; col < size; col++) {
                    ...
                    for(int i = 0; i < size; i++) {
                        total += res[(row * size) + i] * matrix[col + (i * size)];
                    }
...
//    printf("\n");
//
//    printf("Crossing Index: %d\n", flype.crossing);
//    printf("Tangle Length: %d\n", flype.tangle_length);
//
//    printf("Crossing: {");
//    for(int i = 0; i < 4; i++) {
//        printf("%d ", flype.pd_code[4 * flype.crossing + i]);
//    }
//    printf("}\n");
//}
